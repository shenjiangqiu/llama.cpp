    ...
    struct ggml_cgraph *build_llama()
    {
        ...
        for (int il = 0; il < n_layer; ++il)
        {
            ...
            // self-attention
            {
                ...
                // struct ggml_tensor *wq = ggml_save(ctx0, model.layers[il].wq);
                // ggml_set_name(wq, " wqsjq");
                ...
                // struct ggml_tensor *wk = ggml_save(ctx0, model.layers[il].wk);
                // ggml_set_name(wk, " wksjq");
                ...
                // struct ggml_tensor *wv = ggml_save(ctx0, model.layers[il].wv);
                // ggml_set_name(wv, " wvsjq");
...
private:
    ...
    std::vector<std::string> bpe_gpt2_preprocess(const std::string &text)
    {
        ...
        for (int i = 0; i < (int)text_utf.size(); i++)
        {
            ...
            {
                // 're|'ve|'ll
                if (utf_char == "\'" && ((utf_char_next == "r" && utf_char_next_next == "e") ||
...
static ggml_type get_k_quant_type(quantize_state_internal &qs, ggml_type new_type, const ggml_tensor *tensor, llama_ftype ftype)
{
    ...
    else if (name.find("ffn_down.weight") != std::string::npos)
    {
        ...
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_M)
        {
            ...
            {
                new_type = qs.i_feed_forward_w2 < 2 ? GGML_TYPE_Q6_K : use_more_bits(qs.i_feed_forward_w2, qs.n_feed_forward_w2) ? GGML_TYPE_Q5_K
                                                                                                                                 : GGML_TYPE_Q4_K;
            }
            else
            {
        ...
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q5_K_M && use_more_bits(qs.i_feed_forward_w2, qs.n_feed_forward_w2))
            new_type = GGML_TYPE_Q6_K;
